...
const TopicsSection = () => {
  return <section id="topics" className="py-16 md:py-24 film-grain bg-[#cd0644]">
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {topics.map(topic => <CardContainer key={topic.category} containerClassName="py-0">
              <CardBody className="bg-black/40 border-2 border-pulp-yellow/40 w-full h-auto rounded-xl p-4">
                ...
                <CardItem translateZ="40" className="flex flex-wrap gap-2 w-full">
                  {topic.items.map(item => <Badge key={item} variant="secondary" className="py-1 bg-pulp-yellow/20 text-pulp-yellow border border-pulp-yellow/30 font-pixel">
                      {item}
                    </Badge>)}
                </CardItem>
              </CardBody>
