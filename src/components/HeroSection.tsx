      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 relative z-10 pt-24 md:pt-32 flex min-h-screen">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-center w-full">
          ...
          <div className="flex flex-col items-start text-left space-y-8">
            <div className="inline-flex items-center justify-center py-2 px-4 rounded-md text-base bg-pulp-yellow/20 text-pulp-yellow font-medium animate-pulse-slow" onMouseEnter={() => setIsHovered(true)} onMouseLeave={() => setIsHovered(false)}>
              <Calendar className={`h-5 w-5 mr-2 ${isHovered ? 'animate-spin' : ''}`} />
              <span className="relative font-pixel tracking-wide">
                NEW EPISODES EVERY WEDNESDAY
                {isHovered && <span className="absolute -bottom-1 left-0 w-full h-0.5 bg-pulp-yellow"></span>}
              </span>
            </div>
            
            <h1 className="text-6xl sm:text-7xl lg:text-8xl font-bold tracking-tight mb-6 text-pulp-yellow pixel-text" style={{
            ...
            <p className="text-3xl sm:text-4xl font-light text-white mb-4 tracking-wide pixel-text-subtitle" style={{
            fontFamily: "'VT323', monospace"
          }}>
              A FILM PODCAST
            </p>
            ...
            <div className="flex flex-col sm:flex-row gap-4 mt-6">
              <Button size="lg" className="text-lg px-8 py-6 gap-2 bg-pulp-yellow text-black hover:bg-pulp-yellow/90 pixel-button transform hover:scale-105 transition-all font-pixel" onClick={handleLatestEpisodeClick}>
                <Play className="h-5 w-5" />
                LATEST EPISODE
              </Button>
              <Button size="lg" variant="outline" className="text-lg px-8 py-6 border-2 border-white text-white hover:bg-white/10 pixel-button transform hover:scale-105 transition-all font-pixel" onClick={handleSubscribeClick}>
          ...
          <div className="hidden lg:flex items-center justify-center">
            <div className="relative w-full h-[600px] flex justify-center">
              <img alt="Pulp Kitchen Hosts in 8-bit style" className="h-full animate-float-slow object-cover" src="/lovable-uploads/0d6bb033-68cd-47ed-905c-ae2aa7948e1a.png" />
            </div>
          </div>
        </div>
      </div>
